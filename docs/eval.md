...
spec — `nh bench run --trials N` (`src/no_human/cli/commands.py:bench_run:7930`), with a
spec-major fan-out (`:bench_run:8081`) and `(task_id, trial)` as the checkpoint
identity (`:bench_run:7959`) so a resumed multi-trial run cannot double-count a spec.
