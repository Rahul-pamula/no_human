...
| `test_try_kill_still_signals_on_posix` | `signal.SIGKILL` missing on Windows → `AttributeError` at `cli/commands.py:7127` — the exact shape the neighbouring `test_stop_path_never_names_sigkill_at_module_scope` exists to fence |
